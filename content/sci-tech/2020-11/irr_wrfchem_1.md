...
da = ds.assign_coords(RXN=['RXN_'+i.zfill(2) for i in ((ds.RXN)+1).values.astype('str')]).sel(RXN=reaction_names)

# set the IRR to negative if O3 is the reactant
for rxn in da['IRR'].RXN.values:
    if rxn in reactant_names:
        da['IRR'].loc[dict(RXN=rxn)] *= -1
...
large_contrib = da['IRR'].sum(dim='TSTEP').sortby(abs(da['IRR']).sum(dim='TSTEP'), ascending=False).RXN[:8].values
irr_subset = da['IRR'].sel(RXN=large_contrib)
...
# plot the total IRR
total_line = axs.step(da['IRR'].sum(dim='RXN'), where='post', label='Total Chem', color='k')
